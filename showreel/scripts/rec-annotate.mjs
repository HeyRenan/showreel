...
export function makeAnnotator(rctx) {
  ...
  const showAnnotations = async (box, step, sel) => {
    ...
    return await safeEval(({ box, step, modal, modalExplicit, theme, sel, accent }) => {
      ...
      // luma weights match every other lum() in this file (Rec.601) so the
      // contrast floor and the markers judge "light vs dark surface" the same way.
      const __toRGB = (c) => {
        if (!c) return null;
      ...
      const safeAccent = (col, surfaceLum) => {
        ...
        const cl = (0.299 * rgb[0] + 0.587 * rgb[1] + 0.114 * rgb[2]) / 255;
  ...
  const applyHighlight = async (sel, color) => {
    await safeEval((s, col) => {
      ...
      document.querySelectorAll(s).forEach((el) => {
        ...
        // CLIP THE GLOW TO THE CARD: the highlight band's ring + ink overhang
        // project a few px beyond the target on every side, and under a camera
        // zoom that bleeds an amber/red haze OUT of the containing card when the
        // target sits near a card edge. Give the card `overflow:hidden` for the
        // life of the highlight so nothing the band paints can escape it, then
        // restore it in clearMasks. `hidden` (not `clip`) is required: under the
        // camera's <body> transform `clip` does NOT contain the shadow, while
        // `hidden` does. Only patched when the card is currently overflow:visible
        // — a card that manages its own overflow (a scroll region) is left alone
        // so we never trap its content.
        const clipHost = el.closest('.panel,.card,section,[data-sr-clip]');
        if (clipHost && clipHost !== el && !clipHost.dataset.srClipPatched) {
          const ov = getComputedStyle(clipHost).overflow;
          if (ov === 'visible') {
            clipHost.style.overflow = 'hidden';
            clipHost.dataset.srClipPatched = '1';
            void clipHost.offsetWidth; // re-clip the already-composited zoom layer
          }
        }
        const ti = textInset(el, r);
        ...
        // glow brightens the highlight — a soft OUTSET halo that reads as a
        // deliberate spotlight (an inset glow looks flat, like a filled form
        // field). An outset glow would bleed past the card under a camera zoom,
        // so the card is clipped (overflow:hidden + reflow above) for the life of
        // the highlight: the glow stays a real outer glow yet can never escape the
        // card. The thin outer ring is a solid stroke.
        ...
        band.style.cssText = 'position:absolute;top:-' + overY.toFixed(1) + 'px;bottom:-' + overY.toFixed(1) + 'px;left:' + lIn + 'px;right:' + rIn + 'px;z-index:2147483600;pointer-events:none;border-radius:6px;opacity:0;transition:opacity .35s ease;'
          + 'box-shadow:0 0 0 ' + ring.toFixed(1) + 'px ' + (col || (dark ? 'rgba(250,204,21,0.9)' : 'rgba(202,138,4,0.8)')) + ',0 0 ' + (glow * 1.4).toFixed(1) + 'px ' + (glow * 0.4).toFixed(1) + 'px ' + (col || 'rgba(250,204,21,0.55)') + ';';
  ...
  const clearMasks = async () => {
    ...
    await safeEval(() => {
      ...
      document.querySelectorAll('[data-sr-clip-patched]').forEach((el) => { el.style.overflow = ''; delete el.dataset.srClipPatched; });
    });
  };
...
const applyRipple = async (sel, color, opts) => {
  ...
  const applyShake = async (sel, color, opts) => {
      ...
      await safeEval(({ s, col, DUR, SC, INT }) => {
        ...
        // CLIP THE HALO TO THE CARD: with the halo anchored inside the target's
        // subtree, the card's overflow:hidden contains it under a camera zoom —
        // the same fix applyHighlight uses. Patch a card that is currently
        // overflow:visible for the life of the shake; clearMasks restores it. A
        // card that manages its own overflow (a scroll region) is left alone.
        const clipHost = el.closest('.panel,.card,section,[data-sr-clip]');
        if (clipHost && clipHost !== el && !clipHost.dataset.srClipPatched
            && getComputedStyle(clipHost).overflow === 'visible') {
          clipHost.style.overflow = 'hidden';
          clipHost.dataset.srClipPatched = '1';
          void clipHost.offsetWidth; // re-clip the already-composited zoom layer
        }
        ...
        // GLASS ERROR HALO. The host MUST be a CHILD of the target (or its own
        // subtree) so it inherits any ancestor camera transform AND is clipped by
        // the card's overflow:hidden under a zoom — anchoring it to a wide
        // offsetParent instead lets the halo escape the card (it sits in a box far
        // wider than the target under a 2x zoom). Truly replaced controls
        // (input/select/textarea/img/video/canvas) cannot render children, so for
        // those alone we fall back to the nearest block ancestor; a <button> DOES
        // accept children, so it anchors to itself.
        const replaced = /^(INPUT|SELECT|TEXTAREA|IMG|VIDEO|CANVAS)$/.test(el.tagName);
