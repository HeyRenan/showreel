...
[![tests](https://img.shields.io/badge/tests-247%20passing-brightgreen)](#tests)
...
[![Watch the full showcase](assets/showcase-poster.png)](https://github.com/HeyRenan/showreel/releases/download/v1.0.0/showcase.mp4)

*One take, every feature — drawer, camera, loupe, spotlight, live blur, typed form, region pick, one-click deploy. Generated entirely by the plugin against the bundled demo page. ([click to play the full MP4](https://github.com/HeyRenan/showreel/releases/download/v1.0.0/showcase.mp4))*
...
| **Offline render** | `rec.mjs --offline` | Renders on the page's virtual clock — reading dwells collapse, text-heavy takes finish in a fraction of the time. Same steps, same output. |
...
**31 step keys** (unknown keys rejected up front): `click, scrollTo, wait, note, arrow, badge, rect, circle, blur, hide, glide, modal, marks, screen, zoom, topbar, bottombar, fill, text, delay, select, option, camera, glossary, stagger, accent, inset, follow, fade, spotlight, speed`.
...
```bash
cd showreel && node --test 'scripts/__tests__/*.test.mjs'   # 247 tests, no network or browser
```
